...
class OptionsWidget(QWidget):
    ...
    def connect_widget(self, key):
        ...
        if self.dct[key]['type'] in (int, 'int', float, 'float'):
            widget.valueChanged.connect(lambda value: self.set_value(key, value))
        elif self.dct[key]['type'] in (str, 'str'):
            widget.textChanged.connect(lambda value: self.set_value(key, value))
        elif self.dct[key]['type'] in (bool, 'bool'):
            widget.clicked.connect(lambda value: self.set_value(key, value))
        elif self.dct[key]['type'] == 'combo':
            widget.currentIndexChanged.connect(lambda value: self.set_value(key, value))
        elif self.dct[key]['type'] == 'file':
            self.connect_file_dialog(key, widget, self.dct[key].get('save', False), self.dct[key].get('caption', ''),
                                     self.dct[key].get('directory', ''), self.dct[key].get('filter', ''))
        elif self.dct[key]['type'] == 'button':
            widget.clicked.connect(lambda value: self.set_value(key, value))
        if self.dct[key]['type'] == 'size':
            widget.widget_1.valueChanged.connect(lambda value: self.set_value(key, (value, widget.widget_2.value())))
            widget.widget_2.valueChanged.connect(lambda value: self.set_value(key, (widget.widget_1.value(), value)))
    ...
    def connect_file_dialog(self, key, widget, save, caption, directory, filter):
        def triggerd(*args):
            if save:
            ...
            if file:
                ...
                self.set_value(key, file)
        widget.widget_1.textChanged.connect(lambda value: self.set_value(key, value))
